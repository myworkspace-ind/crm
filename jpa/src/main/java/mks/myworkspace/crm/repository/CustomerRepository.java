...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
	...
	@Query("SELECT ms.id, COUNT(c.id) " + "FROM Customer c LEFT JOIN c.mainStatus ms " + "WHERE ms IS NOT NULL "
			+ "GROUP BY ms.id")
	List<Object[]> countCustomersByMainStatus();

	@Query("SELECT ss.id, COUNT(c.id) " + "FROM Customer c LEFT JOIN c.subStatus ss " + "WHERE ss IS NOT NULL "
			+ "GROUP BY ss.id")
	...
	@Query("SELECT c FROM Customer c "
	        ...
	        + "WHERE ci.interactAt BETWEEN :startDate AND :endDate")
